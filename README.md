...
The service will be accessible on port 9090.
...
> ```bash
>  curl -X GET -H "Content-Type: application/json" http://localhost:9090/version
> ```
...
> ```bash
> curl -X POST -H "Content-Type: application/html" --data @input_html http://localhost:9090/convert/html/to/docx --output output.docx
> ```
