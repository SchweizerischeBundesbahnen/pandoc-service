...
This is a port of the ChromiumManager from weasyprint-service, kept deliberately
close to its counterpart (persistent browser, semaphore-bounded concurrency,
per-conversion timeout, bounded retries with browser restart on failure, psutil
resource monitoring, Prometheus metrics and background health monitoring) so the
shared "SVG conversion" logic can later be extracted into a common library.

The browser is Playwright's bundled Chromium, installed in the image via
``playwright install chromium`` and located through PLAYWRIGHT_BROWSERS_PATH.
"""
...
class ChromiumManager:
    ...
    async def _start_internal(self) -> None:
        ...
        try:
            ...
            try:
                # Playwright does not expose the browser PID directly, so locate
                # the headless Chromium process via pgrep for psutil monitoring.
                if self._browser:
    ...
    async def convert_svg_to_png(self, svg_content: str, width: int, height: int, device_scale_factor: float | None = None) -> bytes:
        ...
        async with self._counter_lock:
            ...
            if 0 < self.restart_after_n_conversions <= self._conversion_count:
                ...
                # Reset counter immediately inside lock to prevent other tasks from also triggering restart
                self._conversion_count = 0
...
<body>
    ...
    async def _health_monitor_loop(self) -> None:
        ...
        try:
            while not self._shutdown_event.is_set():
                ...
                if is_healthy:
                    consecutive_failures = 0
                    self.log.debug("Health check passed (uptime: %.1fs, svg conversions: %d, errors: %d)", self._metrics.uptime_seconds, self._metrics.total_svg_conversions, self._metrics.failed_svg_conversions)
                else:
    ...
    def get_metrics(self) -> dict[str, float | int | bool | str]:
        ...
        Returns:
            Dictionary containing current SVG conversion, browser health, resource
            usage and queue metrics. (The HTML-to-PDF fields are retained for
            parity with weasyprint-service and stay at zero here, since this
            service only performs SVG-to-PNG conversions.)
        """
        self._metrics.update_uptime()
        ...
        if self._metrics.last_health_check > 0:
            dt = datetime.fromtimestamp(self._metrics.last_health_check)  # noqa: DTZ006
    ...
    def _validate_int_config(self, value: int | None, env_var: str, default: int, min_value: int, max_value: int) -> int:
        """Validate integer configuration parameters from a value or environment variable."""
        value = self._parse_int(os.environ.get(env_var), default) if value is None else int(value)
    ...
    def _validate_float_config(self, value: float | None, env_var: str, default: float, min_value: float, max_value: float) -> float:
        """Validate float configuration parameters from a value or environment variable."""
        value = self._parse_float(os.environ.get(env_var), default) if value is None else float(value)
    ...
    def _validate_device_scale_factor(self, value: float | None) -> float:
        """Validate device scale factor (1.0 - 10.0)."""
        return self._validate_float_config(value=value, env_var="DEVICE_SCALE_FACTOR", default=1.0, min_value=1.0, max_value=10.0)

    def _validate_max_concurrent_conversions(self, value: int | None) -> int:
        """Validate max concurrent conversions (1 - 100)."""
        return self._validate_int_config(value=value, env_var="MAX_CONCURRENT_CONVERSIONS", default=10, min_value=1, max_value=100)

    def _validate_restart_after_n_conversions(self, value: int | None) -> int:
        """Validate restart after N conversions threshold (0 - 10000)."""
        return self._validate_int_config(value=value, env_var="CHROMIUM_RESTART_AFTER_N_CONVERSIONS", default=0, min_value=0, max_value=10000)

    def _validate_max_conversion_retries(self, value: int | None) -> int:
        """Validate max conversion retry attempts (1 - 10)."""
        return self._validate_int_config(value=value, env_var="CHROMIUM_MAX_CONVERSION_RETRIES", default=2, min_value=1, max_value=10)

    def _validate_conversion_timeout(self, value: int | None) -> int:
        """Validate conversion timeout in seconds (5 - 300)."""
        return self._validate_int_config(value=value, env_var="CHROMIUM_CONVERSION_TIMEOUT", default=30, min_value=5, max_value=300)

    def _validate_health_check_interval(self, value: int | None) -> int:
        """Validate health check interval in seconds (10 - 300)."""
        return self._validate_int_config(value=value, env_var="CHROMIUM_HEALTH_CHECK_INTERVAL", default=30, min_value=10, max_value=300)

    def _validate_health_check_enabled(self, value: bool | None) -> bool:
        """Validate health check enabled flag (default True)."""
