...
DEFAULT_METRICS_PORT = 9182
STARTUP_TIMEOUT_SECONDS = 10.0

# Minimal FastAPI app for metrics only
metrics_app = FastAPI(
    title="Pandoc Metrics",
    version="1.0.0",
    docs_url=None,
    redoc_url=None,
    openapi_url=None,
...
async def metrics() -> Response:
    ...
    - Custom conversion metrics (success/failure counts, durations)
    - Request/response size histograms
    - Service uptime and active conversions
    ...
    to reflect current state (uptime, active conversions, error rate, etc.).
    """
...
def get_metrics_port() -> int:
    ...
    Returns:
        Port number from METRICS_PORT env var (default: 9182).
...
class MetricsServer:
    ...
    def __init__(self, port: int = DEFAULT_METRICS_PORT) -> None:
        ...
        Args:
            port: Port to listen on (default: 9182)
        """
        self.port = port
