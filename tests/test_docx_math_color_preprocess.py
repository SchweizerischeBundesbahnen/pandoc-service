...
from app import docx_math_color_pre_process
...
def _texts(blob: bytes) -> list[str | None]:
    root = ET.fromstring(_unpack(blob))
...
def test_colored_run_is_wrapped_and_color_stripped() -> None:
    out = docx_math_color_pre_process.preprocess(_pack(_doc(_run("E", color="FF0000"))))
...
def test_lowercase_and_hash_hex_normalized_to_uppercase() -> None:
    out = docx_math_color_pre_process.preprocess(_pack(_doc(_run("x", color="#00ffcc"))))
...
def test_multiple_runs_each_wrapped_with_own_color() -> None:
    ...
    out = docx_math_color_pre_process.preprocess(_pack(_doc(body)))
...
def test_run_without_color_is_untouched() -> None:
    ...
    out = docx_math_color_pre_process.preprocess(blob)
...
def test_math_run_with_rpr_but_no_color_is_skipped() -> None:
    ...
    blob = _pack(_doc(body))
    assert docx_math_color_pre_process.preprocess(blob) == blob
...
def test_invalid_zip_returned_unchanged() -> None:
    assert docx_math_color_pre_process.preprocess(b"not a zip") == b"not a zip"
