...
# Own image tag and container name: the shared cleanup of tests/conftest.py removes
# everything tagged with the regular test image, and this container has to survive
# until its logs are read.
IMAGE_TAG = "pandoc_service_shutdown_test"
CONTAINER_NAME = "pandoc_service_shutdown_test"
# Docker sends SIGKILL after this many seconds. The service has to be gone well before.
STOP_TIMEOUT_SECONDS = 30
MAX_SHUTDOWN_SECONDS = 20
READY_TIMEOUT_SECONDS = 120
# uvicorn re-raises the signal it captured once the shutdown is done, so a service stopped
# by SIGTERM reports 143 (128 + 15). 0 covers a uvicorn version which returns instead.
EXPECTED_EXIT_CODES = (0, 143)


def _wait_for_startup(container: Container, max_wait: int = READY_TIMEOUT_SECONDS) -> None:
    """Wait until the application reports a finished startup in its log."""
    start = time.time()
    while time.time() - start < max_wait:
        if "Application startup complete" in container.logs().decode("utf-8"):
            return
        time.sleep(1)
    raise TimeoutError(f"Service did not start within {max_wait}s. Logs:\n{container.logs().decode('utf-8')}")


@pytest.fixture(scope="module")
def stopped_container():
    """
    Build and start the image, wait until it serves, then stop it with SIGTERM.

    The container keeps its logs after the stop (no auto_remove), so the test can read
    what the service did while it shut down.
    ...
    container = client.containers.run(
        ...
        detach=True,
        name=CONTAINER_NAME,
        # No port is published: the test drives the container through Docker only. This
        # keeps it off a host port another test or service may hold.
        init=True,
        auto_remove=False,
        labels={"test-suite": "pandoc-service-shutdown"},
    )
    try:
        _wait_for_startup(container)
...
def test_sigterm_shuts_the_service_down_gracefully(stopped_container) -> None:
    """SIGTERM runs the shutdown hooks: Chromium browser and metrics server are closed."""
