"""Unit tests for ``app.docx_list_level_pre_process``.
...
from app import docx_list_level_pre_process
from app.docx_list_level_pre_process import SENTINEL_CLOSE, SENTINEL_OPEN
...
def test_list_paragraph_gets_sentinel_for_its_ilvl():
    ...
    result = docx_list_level_pre_process.preprocess(blob)
...
def test_numpr_without_ilvl_defaults_to_level_zero():
    ...
    para = _body(docx_list_level_pre_process.preprocess(blob)).find(f".//{{{W_NS}}}p")
...
def test_multiple_levels_tagged_independently():
    ...
    paras = _body(docx_list_level_pre_process.preprocess(blob)).findall(f".//{{{W_NS}}}p")
...
def test_non_list_paragraph_is_untouched():
    ...
    assert docx_list_level_pre_process.preprocess(blob) == blob
...
def test_preprocess_is_idempotent():
    ...
    once = docx_list_level_pre_process.preprocess(blob)
    # A second pass must not prepend a second sentinel.
    assert docx_list_level_pre_process.preprocess(once) == once
...
def test_non_docx_input_is_returned_unchanged():
    assert docx_list_level_pre_process.preprocess(b"not a zip") == b"not a zip"
...
def test_ilvl_element_without_val_defaults_to_zero():
    ...
    para_el = _body(docx_list_level_pre_process.preprocess(_pack({"word/document.xml": _doc(para)}))).find(f".//{{{W_NS}}}p")
