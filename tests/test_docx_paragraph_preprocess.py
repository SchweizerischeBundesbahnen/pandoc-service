"""Unit tests for ``app.docx_paragraph_pre_process``.
...
from app import docx_paragraph_pre_process
...
def test_plain_paragraph_returns_input_unchanged():
    ...
    assert docx_paragraph_pre_process.preprocess(blob) == blob


def test_paragraph_without_ppr_is_ignored():
    doc = b'<?xml version="1.0" encoding="UTF-8" standalone="yes"?><w:document xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main"><w:body><w:p><w:r><w:t>x</w:t></w:r></w:p></w:body></w:document>'
...
def test_center_alignment_becomes_pstyle_and_jc_stripped():
    ...
    result = docx_paragraph_pre_process.preprocess(blob)
    body = _body(result)
...
def test_right_alignment():
    ...
    body = _body(docx_paragraph_pre_process.preprocess(blob))
...
def test_existing_pstyle_is_replaced_and_pstyle_is_first_child():
    ...
    assert next(iter(ppr)).tag == f"{{{W_NS}}}pStyle"
...
def test_preprocess_is_idempotent():
    ...
    once = docx_paragraph_pre_process.preprocess(blob)
    # Second pass finds no jc/ind left to rewrite -> returns its input unchanged.
    assert docx_paragraph_pre_process.preprocess(once) == once
...
def test_non_docx_input_is_returned_unchanged():
    assert docx_paragraph_pre_process.preprocess(b"not a zip") == b"not a zip"
